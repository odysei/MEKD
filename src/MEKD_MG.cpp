...
/// MEs.
#include "MG5_aMCNLO/DY/MEKD_MEs_DY.h"
#include "MG5_aMCNLO/Z4l/MEKD_MEs_Z4l.h"
#include "MG5_aMCNLO/ZZ/MEKD_MEs_ZZ.h"

#include "MG5_aMCNLO/Spin0/MEKD_MEs_Spin0.h"
#include "MG5_aMCNLO/Spin1/MEKD_MEs_Spin1.h"
#include "MG5_aMCNLO/Spin2/MEKD_MEs_Spin2.h"
...
#include "MG5_aMCNLO/read_slha.h"
...
int MEKD_MG::Load_Parameters()
{
	...
	params_m_Z =
		Set_Of_Model_Parameters.get_block_entry("mass", 23, 9.11876e+01).real();
...
void MEKD_MG::Set_Default_MEKD_MG_Parameters()
{
	...
	ContributionCoeff_d = 0; // 42	/// the value has no effect if PDF is used
	...
	Higgs_width =
		5.753088e-03; // Practically not used, for future implementations
...
#ifndef MEKD_STANDALONE
	string rel_param_path =
		"ZZMatrixElement/MEKD/src/Cards/param_card.dat"; // Location where a
														 // parameter card is
														 // stored
	string rel_pdf_path =
		"ZZMatrixElement/MEKD/src/PDF_tables/cteq6l.pdt"; // PDF/PDT table file
...
int MEKD_MG::Run_MEKD_MG()
{
	...
	if (Use_PDF_w_pT0) {
		...
		// 		Boost_4p_and_2p_2_pT0( ml1, p_set[2], ml2, p_set[3], ml3,
		// p_set[4],
		// ml4, p_set[5], Proton_mass, p_set[0], Proton_mass, p_set[1] );
		// 		Boost_4p_2_pT0( ml1, p_set[2], ml2, p_set[3], ml3, p_set[4],
		// ml4,
		// p_set[5] );
		Boost_5p_2_pT0(ml1, p_set[2], ml2, p_set[3], ml3, p_set[4], ml4,
					   p_set[5], 0, p_set[6]);

		PDFx1 = ((p_set[2][0] + p_set[3][0] + p_set[4][0] + p_set[5][0] +
				  p_set[6][0]) +
				 (p_set[2][3] + p_set[3][3] + p_set[4][3] + p_set[5][3] +
				  p_set[6][3])) /
				(2 * p_set[0][0]);
		PDFx2 = ((p_set[2][0] + p_set[3][0] + p_set[4][0] + p_set[5][0] +
				  p_set[6][0]) -
				 (p_set[2][3] + p_set[3][3] + p_set[4][3] + p_set[5][3] +
				  p_set[6][3])) /
				(2 * p_set[0][0]);
	...
	if (Boost_To_CM && !Use_PDF_w_pT0) {
		// 		Boost2CM( ml1, p_set[2], ml2, p_set[3], ml3, p_set[4], ml4,
		// p_set[5] );
	...
	if (!Use_PDF_w_pT0) {
		buffer = new double;
		(*buffer) = (ContributionCoeff_d + ContributionCoeff_u +
