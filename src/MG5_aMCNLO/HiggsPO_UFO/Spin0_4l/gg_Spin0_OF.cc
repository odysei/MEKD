...
// Process: g g > h WEIGHTED=2 @1
// *   Decay: h > e- e+ mu- mu+ YUK=0

//--------------------------------------------------------------------------
// Initialize process.

void gg_Spin0_OF::initProc(string param_card_name)
{
    ...
    if (sum_hel == 0 || ntry < 10) {
        ...
        for (int ihel = 0; ihel < ncomb; ihel++) {
            if (goodhel[ihel] || ntry < 2) {
                ...
                t[0] = matrix_1_gg_h_h_emepmummup();
...
void gg_Spin0_OF::calculate_wavefunctions(const int perm[], const int hel[])
{
    ...
    FFV2_3_3(w[3], w[2], pars->GC_30, pars->GC_31, pars->mdl_MZ, pars->mdl_WZ,
             w[6]);
    ...
    VVS3_4_5_3(w[6], w[11], pars->GC_20, pars->GC_8, pars->GC_19, pars->mdl_MH,
               pars->mdl_WH, w[14]);
    FFVS1_2_4(w[3], w[2], w[11], pars->GC_12, pars->GC_13, pars->mdl_MH,
              pars->mdl_WH, w[15]);
    ...
    VVS5_0(w[0], w[1], w[15], pars->GC_9, amp[5]);
}
double gg_Spin0_OF::matrix_1_gg_h_h_emepmummup()
{
    int i, j;
    // Local variables
    //   const int ngraphs = 6;
