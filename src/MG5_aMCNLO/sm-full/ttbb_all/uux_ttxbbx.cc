...
// Process: u u~ > t t~ b b~ QED=2 @1

//--------------------------------------------------------------------------
// Initialize process.

void uux_ttxbbx::initProc(string param_card_name) 
{
...
void uux_ttxbbx::calculate_wavefunctions(const int perm[], const int hel[])
{
...
//   int i, j; 
  ...
  FFV2_5_2(w[3], w[16], pars->GC_51, pars->GC_58, pars->mdl_MT, pars->mdl_WT,
      w[30]);
  FFV2_3(w[0], w[4], pars->GC_102, pars->mdl_MW, pars->mdl_WW, w[31]); 
  FFV2_1(w[1], w[31], pars->GC_43, pars->mdl_MB, pars->ZERO, w[32]); 
  FFV2_2(w[5], w[31], pars->GC_43, pars->ZERO, pars->ZERO, w[33]); 
  FFV2_3(w[5], w[1], pars->GC_43, pars->mdl_MW, pars->mdl_WW, w[34]); 
  FFV2_2(w[0], w[34], pars->GC_102, pars->mdl_MB, pars->ZERO, w[35]); 
  FFV1_2(w[0], w[7], pars->GC_11, pars->ZERO, pars->ZERO, w[36]); 
  FFV1_2(w[0], w[11], pars->GC_2, pars->ZERO, pars->ZERO, w[37]); 
  FFV1_2(w[0], w[19], pars->GC_11, pars->ZERO, pars->ZERO, w[38]); 
  FFV1_2(w[0], w[26], pars->GC_2, pars->ZERO, pars->ZERO, w[39]); 
  FFV2_5_2(w[0], w[28], pars->GC_51, pars->GC_58, pars->ZERO, pars->ZERO,
      w[40]);
  ...
  FFV1_0(w[33], w[1], w[7], pars->GC_11, amp[30]); 
  FFV1_0(w[35], w[4], w[7], pars->GC_11, amp[31]); 
  FFV2_0(w[36], w[4], w[34], pars->GC_102, amp[32]); 
  FFV1_0(w[37], w[1], w[19], pars->GC_11, amp[33]); 
  FFV1_0(w[38], w[1], w[11], pars->GC_2, amp[34]); 
  FFV1_0(w[36], w[1], w[26], pars->GC_2, amp[35]); 
  FFV1_0(w[39], w[1], w[7], pars->GC_11, amp[36]); 
  FFV1_0(w[36], w[1], w[19], pars->GC_11, amp[37]); 
  FFV1_0(w[38], w[1], w[7], pars->GC_11, amp[38]); 
  FFV2_5_0(w[36], w[1], w[28], pars->GC_51, pars->GC_58, amp[39]); 
  FFV1_0(w[40], w[1], w[7], pars->GC_11, amp[40]); 
  FFV1_0(w[41], w[1], w[19], pars->GC_11, amp[41]); 
  FFV2_5_0(w[38], w[1], w[15], pars->GC_51, pars->GC_58, amp[42]); 

}
double uux_ttxbbx::matrix_1_uux_ttxbbx() 
{
  int i, j; 
  // Local variables
//   const int ngraphs = 43; 
  ...
  jamp[0] = -1./6. * amp[0] - 1./6. * amp[1] - 1./6. * amp[2] - 1./6. * amp[3]
      ...
      1./6. * amp[17] - 1./6. * amp[18] - 1./6. * amp[19] - 1./6. * amp[20] +
      1./36. * amp[21] + 1./36. * amp[22] - 1./6. * amp[23] - 1./6. * amp[24] -
      1./6. * amp[25] - 1./6. * amp[26] - 1./6. * amp[27] - 1./6. * amp[28] -
      1./6. * amp[33] - 1./6. * amp[34] - 1./6. * amp[35] - 1./6. * amp[36] +
      1./36. * amp[37] + 1./36. * amp[38] - 1./6. * amp[39] - 1./6. * amp[40] -
      1./6. * amp[41] - 1./6. * amp[42];
  jamp[1] = +1./2. * amp[0] + 1./2. * amp[1] - 1./12. * amp[4] - 1./12. *
      amp[5] + 1./2. * amp[10] + 1./2. * amp[11] + 1./6. * amp[13] + 1./6. *
      amp[14] + 1./6. * amp[15] + 1./6. * amp[16] + 1./2. * amp[17] + 1./2. *
      amp[18] - 1./12. * amp[21] - 1./12. * amp[22] + 1./2. * amp[27] + 1./2. *
      amp[28];
  jamp[2] = +1./2. * amp[19] + 1./2. * amp[20] - 1./12. * amp[21] - 1./12. *
      amp[22] + 1./2. * amp[23] + 1./2. * amp[24] + 1./2. * amp[25] + 1./2. *
      amp[26] + 1./2. * amp[35] + 1./2. * amp[36] - 1./12. * amp[37] - 1./12. *
      amp[38] + 1./2. * amp[39] + 1./2. * amp[40];
  jamp[3] = +1./4. * amp[4] - 1./4. * std::complex<double> (0, 1) * amp[12] -
      1./2. * amp[13] - 1./2. * amp[14] + 1./4. * amp[22] - 1./2. * amp[29] -
      1./2. * amp[30] + 1./4. * amp[38];
  jamp[4] = +1./4. * amp[5] + 1./4. * std::complex<double> (0, 1) * amp[12] -
      1./2. * amp[15] - 1./2. * amp[16] + 1./4. * amp[21] - 1./2. * amp[31] -
      1./2. * amp[32] + 1./4. * amp[37];
  jamp[5] = +1./2. * amp[2] + 1./2. * amp[3] - 1./12. * amp[4] - 1./12. *
      amp[5] + 1./2. * amp[6] + 1./2. * amp[7] + 1./2. * amp[8] + 1./2. *
      amp[9] + 1./6. * amp[29] + 1./6. * amp[30] + 1./6. * amp[31] + 1./6. *
      amp[32] + 1./2. * amp[33] + 1./2. * amp[34] - 1./12. * amp[37] - 1./12. *
      amp[38] + 1./2. * amp[41] + 1./2. * amp[42];
