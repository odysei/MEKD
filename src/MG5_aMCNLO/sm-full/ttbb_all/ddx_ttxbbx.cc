...
//   int i, j; 
  ...
  FFV2_5_2(w[3], w[16], pars->GC_51, pars->GC_58, pars->mdl_MT, pars->mdl_WT,
      w[30]);
  FFV2_3(w[0], w[2], pars->GC_47, pars->mdl_MW, pars->mdl_WW, w[31]); 
  FFV2_1(w[1], w[31], pars->GC_106, pars->mdl_MT, pars->mdl_WT, w[32]); 
  FFV2_2(w[3], w[31], pars->GC_106, pars->ZERO, pars->ZERO, w[33]); 
  FFV2_3(w[3], w[1], pars->GC_106, pars->mdl_MW, pars->mdl_WW, w[34]); 
  FFV2_2(w[0], w[34], pars->GC_47, pars->mdl_MT, pars->mdl_WT, w[35]); 
  ...
  FFV2_0(w[36], w[2], w[34], pars->GC_47, amp[32]); 
...
//   const int ngraphs = 43; 
  ...
  jamp[2] = +1./2. * amp[19] + 1./2. * amp[20] - 1./12. * amp[21] - 1./12. *
      ...
      amp[26] + 1./6. * amp[29] + 1./6. * amp[30] + 1./6. * amp[31] + 1./6. *
      amp[32] + 1./2. * amp[35] + 1./2. * amp[36] - 1./12. * amp[37] - 1./12. *
      amp[38] + 1./2. * amp[39] + 1./2. * amp[40];
  jamp[3] = +1./4. * amp[4] - 1./4. * std::complex<double> (0, 1) * amp[12] -
      1./2. * amp[13] - 1./2. * amp[14] + 1./4. * amp[22] - 1./2. * amp[31] -
      1./2. * amp[32] + 1./4. * amp[38];
  jamp[4] = +1./4. * amp[5] + 1./4. * std::complex<double> (0, 1) * amp[12] -
      1./2. * amp[15] - 1./2. * amp[16] + 1./4. * amp[21] - 1./2. * amp[29] -
      1./2. * amp[30] + 1./4. * amp[37];
  jamp[5] = +1./2. * amp[2] + 1./2. * amp[3] - 1./12. * amp[4] - 1./12. *
      amp[5] + 1./2. * amp[6] + 1./2. * amp[7] + 1./2. * amp[8] + 1./2. *
      amp[9] + 1./2. * amp[33] + 1./2. * amp[34] - 1./12. * amp[37] - 1./12. *
