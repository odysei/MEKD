...
void qq_Z4l_DN_OF::initProc(string param_card_name) 
{
...
void qq_Z4l_DN_OF::updateProc(SLHAReader slha) 
{
  pars->setIndependentParameters(slha); 
  pars->setIndependentCouplings();
	ntry = 0, sum_hel = 0, ngood = 0;	// moved here by Ghost remover v. 0.1 
...
void qq_Z4l_DN_OF::sigmaKin_BKG() 
{
...
void qq_Z4l_DN_OF::sigmaKin_SIG() 
{
  // Set the parameters which change event by event
  pars->setDependentParameters(); 
  pars->setDependentCouplings();
...
void qq_Z4l_DN_OF::calculate_wavefunctions_BKG(const int perm[], const int hel[])
{
...
void qq_Z4l_DN_OF::calculate_wavefunctions_SIG(const int perm[], const int hel[])
{
  // Calculate wavefunctions for all processes
//   int i, j; 
...
double qq_Z4l_DN_OF::matrix_ssx_emepmummup_no_hxg() 
{
  int i, j; 
  // Local variables
