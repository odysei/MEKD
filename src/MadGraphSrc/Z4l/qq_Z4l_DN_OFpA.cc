...
void qq_Z4l_DN_OFpA::initProc(string param_card_name) 
{
...
void qq_Z4l_DN_OFpA::updateProc(SLHAReader slha) 
{
  pars->setIndependentParameters(slha); 
  pars->setIndependentCouplings();
	ntry = 0, sum_hel = 0, ngood = 0;	// moved here by Ghost remover v. 0.1 
...
void qq_Z4l_DN_OFpA::sigmaKin_BKG() 
{
...
void qq_Z4l_DN_OFpA::sigmaKin_SIG() 
{
  // Set the parameters which change event by event
  pars->setDependentParameters(); 
  pars->setDependentCouplings(); 
...
double qq_Z4l_DN_OFpA::sigmaHat() 
{
...
void qq_Z4l_DN_OFpA::calculate_wavefunctions_BKG(const int perm[], const int hel[])
{
...
//       w[40]);
  ...
  // Amplitude(s) for diagram number 0
	for( unsigned int count=0; count<167; count++ ) amp[count] = 0;
...
void qq_Z4l_DN_OFpA::calculate_wavefunctions_SIG(const int perm[], const int hel[])
{
  // Calculate wavefunctions for all processes
//   int i, j; 
...
double qq_Z4l_DN_OFpA::matrix_ssx_emepmummupa_no_hzpxg() 
{
  int i, j; 
  // Local variables
