...
void qq_Z4l_UP_SFpA::initProc(string param_card_name) 
{
...
void qq_Z4l_UP_SFpA::updateProc(SLHAReader slha) 
{
  pars->setIndependentParameters(slha); 
  pars->setIndependentCouplings();
	ntry = 0, sum_hel = 0, ngood = 0;	// moved here by Ghost remover v. 0.1 
...
void qq_Z4l_UP_SFpA::sigmaKin_BKG() 
{
...
void qq_Z4l_UP_SFpA::sigmaKin_SIG() 
{
  // Set the parameters which change event by event
  pars->setDependentParameters(); 
  pars->setDependentCouplings(); 
...
double qq_Z4l_UP_SFpA::sigmaHat() 
{
...
void qq_Z4l_UP_SFpA::calculate_wavefunctions_BKG(const int perm[], const int hel[])
{
...
//   FFV2_4_1(w[2], w[16], pars->Unitary_GC_70, pars->Unitary_GC_75, pars->MM, pars->ZERO, w[52]); 
  ...
  // Amplitude(s) for diagram number 0
 	for( unsigned int count=0; count<335; count++ ) amp[count] = 0;
...
void qq_Z4l_UP_SFpA::calculate_wavefunctions_SIG(const int perm[], const int hel[])
{
  // Calculate wavefunctions for all processes
//   int i, j; 
...
double qq_Z4l_UP_SFpA::matrix_ccx_mummupmummupa_no_hzpxg() 
{
  int i, j; 
  // Local variables
