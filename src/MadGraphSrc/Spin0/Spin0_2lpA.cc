...
// Process: h > mu- mu+ a / zp xg

//--------------------------------------------------------------------------
// Initialize process.

void Spin0_2lpA::initProc(string param_card_name) 
{
...
void Spin0_2lpA::updateProc(SLHAReader slha)
{
	pars->setIndependentParameters(slha);
	pars->setIndependentCouplings();
...
//	std::complex<double> * * wfs;	// Changed by Convert_source 0.2
  ...
  static const int helicities[ncomb][nexternal] = {{0, -1, -1, -1}, {0, -1, -1,
...
void Spin0_2lpA::calculate_wavefunctions(const int perm[], const int hel[])
{
...
//	int i, j;	// Changed by Convert_source 0.2
  ...
  FFS1_2_0(w[2], w[4], w[0], pars->HEF_MEKD2_1_GC_137, pars->HEF_MEKD2_1_GC_136, amp[0]);
  FFS1_2_0(w[5], w[1], w[0], pars->HEF_MEKD2_1_GC_137, pars->HEF_MEKD2_1_GC_136, amp[1]);

}
double Spin0_2lpA::matrix_h_mummupa_no_zpxg() 
{
  int i, j;
  // Local variables
