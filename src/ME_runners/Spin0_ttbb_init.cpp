...
bool ME_runner_all_Spin0Pm_2f_ttbb_2::initialize(const string &param_f)
{
    if (!ME_Initiator(param_f, "ME_ddx", 2,
                      new MG5_heft_updated_full::ddx_h_ttxbbx(), ME_ddx))
        return false;
    if (!ME_Initiator(param_f, "ME_uux", 2,
                      new MG5_heft_updated_full::uux_h_ttxbbx(), ME_uux))
        return false;
    if (!ME_Initiator(param_f, "ME_ssx", 2,
                      new MG5_heft_updated_full::ssx_h_ttxbbx(), ME_ssx))
        return false;
    if (!ME_Initiator(param_f, "ME_ccx", 2,
                      new MG5_heft_updated_full::ccx_h_ttxbbx(), ME_ccx))
        return false;
    if (!ME_Initiator(param_f, "ME_bbx", 2,
                      new MG5_heft_updated_full::bbx_h_ttxbbx(), ME_bbx))
        return false;
    if (!ME_Initiator(param_f, "ME_gg", 1,
