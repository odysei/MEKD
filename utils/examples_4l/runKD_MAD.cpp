...
int calculateKD(string inputFile, string tree, string resonanceName,
				string pdfInclude, string logName);
int calculateKD_Madgraph(string inputFile, string resonanceName,
						 string pdfInclude, string logName);
int calculateKD_Madgraph(string file, string tree, string resonanceName,
						 string pdfInclude, string logName);
...
int main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "f:t:s:x:p:l:?:h")) != -1) {
		...
		case 'f':
			...
			break;
		case 't':
			inputTree = string(optarg);
			break;
		case 's':
			ppEnergy = atof(optarg);
			break;
		case 'x':
			resonanceName = string(optarg);
			break;
		case 'p':
			pdfInclude = string(optarg);
			break;
		case 'l':
		...
		case 'h':
			...
			break;
		default:
			cerr << "\n[ MEKD - ERROR: Unknown option '" << optarg << "' ]"
				 << endl << endl;
			usage(-1);
		}
	}
	...
	// Basic sanity checks
	// Basic sanity checks
	if (inputFile == "" || resonanceName == "")
		usage(-1);
	if (pdfInclude != "CTEQ6L")
		pdfInclude = "no PDFs"; // only CTEQ6L supported at the moment

	// Print chosen options
	cout << endl;
	cout << "----------------------------------------------------------"
		 << endl;
	cout << " Matrix Element (ME) Kinematic Discriminant (KD) producer "
		 << endl;
	cout << "----------------------------------------------------------"
		 << endl;
	...
	int err =
		calculateKD(inputFile, inputTree, resonanceName, pdfInclude, logName);
...
void usage(int status)
{
	cout << "------------------------------------------------------------------"
			"----------------------------------------" << endl;
	cout << "                        Matrix Element (ME) Kinematic "
			"Discriminant (KD) producer                          " << endl;
	cout << "------------------------------------------------------------------"
			"----------------------------------------" << endl;
	cout << "  Usage:                                                          "
			"                                        " << endl;
#ifdef MEKD_with_ROOT
	cout << "     ./runKD_MAD [-f input_file] [-t root_tree] [-s sqrts] [-x "
			"x_resonance] [-p pdf_include] [-l log_file]" << endl;
#else
	cout << "     ./runKD_MAD [-f input_file] [-s sqrts] [-x x_resonance] [-p "
			"pdf_include] [-l log_file]               " << endl;
#endif
	cout << endl;
	cout << "  where:" << endl;
	cout << "     input_file    name of the input file (string, REQUIRED)      "
			"                              " << endl << endl;
#ifdef MEKD_with_ROOT
	cout << "     root_tree     name of the input root tree (string, DEFAULT = "
			"'passedEvents')               " << endl << endl;
	cout << "                   Available only if compiled with ROOT.          "
			"                              " << endl;
#endif
	cout << "     sqrts         pp collsiion energy in TeV (double, DEFAULT = "
			"8)                             " << endl;
	cout << "                   Available options: 7, 8.                       "
			"                              " << endl << endl;
	cout << "     x_resonance   choice of the signal spin-0 resonance (string, "
			"DEFAULT = 'SMHiggs')          " << endl;
	cout << "                   Available options: 'SMHiggs', 'Higgs0M', "
			"'Graviton2PM' and 'Custom'.         " << endl << endl;
	cout << "     pdf_include   Name of PDFs, PDFs not used if pdf_include = "
			"'' (string, DEFAULT= 'CTEQ6L')  " << endl;
	cout << "                   Available PDFs: 'CTEQ6L' (NONE = '').          "
			"                              " << endl << endl;
	cout << "     log_file      name of the log file, no logging if log_file = "
			"'' (string, DEFAULT = '')     " << endl;
	cout << endl;
	cout << "------------------------------------------------------------------"
			"----------------------------------------" << endl;
...
int redirectAndSaveStdout(fpos_t &pos, string logName)
{
	...
	if (logName == "")
		...
		freopen((const_cast<char *>(logName.c_str())), "w", stdout);
	return fd;
}
...
void readLeptonIDsAnd4Momenta(ifstream &data, int &idL1, int &idL2, int &idL3,
							  int &idL4, double p3in[], double p4in[],
							  double p5in[], double p6in[])
{
...
int calculateKD(string inputFile, string inputTree, string resonanceName,
				string pdfInclude, string logName)
{
	// Check if a supported resonance name is provided and run KD calculator
	// based on the library meMadgraph.h (which uses some Madgraph libraries)
	if (resonanceName == "SMHiggs" || resonanceName == "Higgs0M" ||
		resonanceName == "Graviton2PM" || resonanceName == "Custom") {
		if (inputFile.substr(inputFile.length() - 5, inputFile.length() - 1) ==
			".root") {
#ifdef MEKD_with_ROOT
			return calculateKD_Madgraph(inputFile, inputTree, resonanceName,
										pdfInclude, logName);
#else
			...
			return calculateKD_Madgraph(inputFile, resonanceName, pdfInclude,
										logName);
	} else {
		cerr << "\n[ MEKD - ERROR: Resonance of type '" + resonanceName +
...
//------------------------------------------------------------------------------------------------------------
// Read the kinematic information for sample of events from the input file and
// calculate MEs and KD
int calculateKD_Madgraph(string inputFile, string resonanceName,
						 string pdfInclude, string logName)
{
	...
	if (!data.good()) {
		cerr << "\n[ MEKD - ERROR: File '" + inputFile + "' is not readable. ]"
			 << endl << endl;
		return -1;
	}
	...
	while (!(data.eof())) {
		...
		cout << "resonanceName: " << resonanceName
			 << ", backgroundName: " << backgroundName << endl;

		// Calclualte MEs and KD
		MEKDwithPDFs->computeKD(resonanceName, backgroundName, p1, idL1, p2,
								idL2, p3, idL3, p4, idL4, KD, ME_XtoZZ, ME_ZZ);
						 ...
						 string pdfInclude, string logName)
{
	...
	TFile f(inputFile);
	if (f.IsZombie()) {
		cerr << "\n [ MEKD - ERROR: File '" + inputFile + "' does not exist ]"
			 << endl << endl;
		return -1;
	}

	TFile *newFile = new TFile(
		inputFile(0, inputFile.Length() - 5) + "_withDiscriminant.root",
		"RECREATE"); // replace '.root' with '_withDiscriminants.root'
	...
	if (!sigTree) {
		cerr << "\n [ MEKD - ERROR: Can not find the tree '" + treeName + "' ]"
			 << endl << endl;
		return -1;
	}
	...
	TBranch *me_XtoZZ =
		sigTree->Branch("ME_HiggsSM", &ME_XtoZZ, "ME_HiggsSM/D");
	...
	for (int iEvt = 0; iEvt < nentries; iEvt++) {
		...
		// compute MEs and KD
		int err = MEKDwithPDFs->computeKD(resonanceName, backgroundName, p1,
										  idL1, p2, idL2, p3, idL3, p4, idL4,
										  KD, ME_XtoZZ, ME_ZZ);
