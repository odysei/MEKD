...
int MEKD_Test_Check_Models_Test2()
{
    ...
    for (unsigned int count_model = 0; count_model < models_to_test.size();
         count_model++) {
        if ((error_value =
                 test2.computeME(models_to_test[count_model], Set_Of_Arrays,
                                 Set_Of_IDs, ME_value)) != 0)
            cout << "ERROR CODE in ME for " << models_to_test[count_model]
                 << ": " << error_value << endl;
        else
            cout << setw(30) << left << "Result for " << setw(15) << left
        ...
        if ((error_value =
                 test2.computeME(models_to_test[count_model], Set_Of_Arrays_pA,
                                 Set_Of_IDs_pA, ME_value)) != 0)
            cout << "ERROR CODE in ME for " << models_to_test[count_model]
                 << ": " << error_value << endl;
        else
            cout << setw(30) << left << "Result for (with a photon)" << setw(15)
