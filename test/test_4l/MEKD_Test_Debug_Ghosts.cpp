...
int MEKD_Test_Debug_Ghosts_Test0()
{
    ...
    if (Show_Description)
        cout << "Testing ME ordering using int computeME( string, "
                "vector<double*>, vector<int>, double& );\n";

    if ((error_value =
             test01.computeME(model[0], Set_Of_Arrays, Set_Of_IDs, ME[0])) != 0)
        cout << "ERROR CODE in ME for " << model[0] << "; Err: " << error_value
             << endl;
    if ((error_value =
             test01.computeME(model[1], Set_Of_Arrays, Set_Of_IDs, ME[1])) != 0)
        cout << "ERROR CODE in ME for " << model[1] << "; Err: " << error_value
             << endl;
    if ((error_value =
             test02.computeME(model[2], Set_Of_Arrays, Set_Of_IDs, ME[2])) != 0)
        cout << "ERROR CODE in ME for " << model[2] << "; Err: " << error_value
             << endl;
    if ((error_value =
             test02.computeME(model[3], Set_Of_Arrays, Set_Of_IDs, ME[3])) != 0)
        cout << "ERROR CODE in ME for " << model[3] << "; Err: " << error_value
             << endl;

    if (Show_Basic_Data) {
        if (Show_Description)
            cout << "Initial results, before any permutation.\n";
        cout.width(20);
        cout << std::left << "Model" << std::right << "ME value\n\n";

        for (unsigned int count = 0; count < Nr_of_models; count++) {
            cout.width(20);
            cout << std::left << model[count] << std::right << ME[count]
                 << endl;
        }
    }
...
int MEKD_Test_Debug_Ghosts_Test1()
{
    ...
    for (unsigned int count = 0; count < Nr_of_models; count++) {
        if ((error_value = test1.computeME(model[count], Set_Of_Arrays,
                                           Set_Of_IDs, ME[count])) != 0)
            cout << "ERROR CODE in ME for " << model[count]
                 << "; Err: " << error_value << endl;
    }
    ...
    for (unsigned int perm = 0; perm < shuffles_for_ghosts; perm++) {
        ...
        for (unsigned int count = 0; count < Nr_of_models; count++) {
            if ((error_value = test1.computeME(model[order[count]],
                                               Set_Of_Arrays, Set_Of_IDs,
                                               ME_shuffled[order[count]])) != 0)
                cout << "ERROR CODE in ME for " << model[order[count]]
                     << "; Err: " << error_value << endl;

            if ((ME[order[count]] - ME_shuffled[order[count]]) != 0) {
                if (Show_Basic_Data) {
                    cout << "Problem in moving order of calculation for "
                         << model[order[count]] << " detected!\n";
                    cout << "Previously calculated models in this shuffle: ";
                    for (unsigned int count2 = 0; count2 < count; count2++)
                        cout << model[order[count2]] << " ";
                }
                return 1;
            }
        }
        ...
        if (Show_Debug) {
            if (Show_Description)
                cout << "Results after a permutation.\n";
            cout.width(20);
            cout << std::left << "Model" << std::right << "ME value\n\n";

            for (unsigned int count = 0; count < Nr_of_models; count++) {
                cout.width(20);
                cout << std::left << model[count] << std::right
                     << ME_shuffled[count] << endl;
            }
        }
    }
...
int MEKD_Test_Debug_Ghosts_Test2()
{
    ...
    if ((error_value = test2.Run()) != 0)
        cout << "ERROR CODE in MEKD.Run(); Err: " << error_value << endl;
    ...
    for (unsigned int perm = 0; perm < shuffles_for_ghosts; perm++) {
        ...
        for (unsigned int count = 0; count < Nr_of_models; count++)
            ME_shuffled[order[count]] = test2_perm->Signal_MEs[count];
